 ...
 * 请求），因此测试开始前先确保默认模型有 key（DeepSeek 默认模型，配置
 * 内联在 cleanup.ts 的 E2E_DEFAULT_MODEL，api_key 取 LLM_API_KEY 环境
 * 变量或占位 key），否则无 key 环境（CI / 新机器）所有聊天 E2E 会被
 * preflight 拒绝。后端未就绪时静默跳过——前置 project
 * （preflight-unconfigured.spec）收尾也会配回 key。
 */
