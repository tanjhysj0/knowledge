...
class Document(Base):
    ...
    # #62/#63：处理状态与进度（上传与索引分离）。
    # 上传落库即 pending/0，后台任务流转 processing → ready（100）或
    # failed；server_default 让 fresh 库 create_all 与存量库内联迁移的
    # DEFAULT 子句语义一致（存量行回填 ready/100）。
    status = Column(
    ...
    # #63：索引处理失败原因（status=failed 时写入）；可空，处理成功/存量记录为空。
