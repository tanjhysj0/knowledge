"""#66：Dense 检索器——复用现有 bge-m3 embedding + Milvus COSINE 检索。
...
# 迁移自 rag.RETRIEVAL_SCORE_THRESHOLD：pymilvus 对 COSINE metric 的
# ``distance`` 字段实为余弦相似度（越大越相关），低于该阈值视为假命中。
SCORE_THRESHOLD = 0.5


class DenseRetriever:
    """Milvus dense 向量检索（策略名 ``dense``）。"""
    ...
    def _search_sync(
        ...
        try:
            ...
            # mock provider 短路：零向量在 Milvus COSINE 下会误命中。
        ...
        except Exception:  # noqa: BLE001 — 向量库不可用时降级为空
            return []
